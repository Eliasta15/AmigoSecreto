...
@admin_required
def realizar_sorteo():
    """Realiza sorteo solo para participantes NUEVOS (que no han participado antes)"""
    conn = psycopg2.connect(**DB_CONFIG)
    cur = conn.cursor()
    
    try:
        # Obtener participantes NUEVOS (que no han participado en sorteos)
        cur.execute("""
            SELECT id, nombre 
            FROM ia.tb_participantes_ai 
            WHERE participo_sorteo = FALSE 
            ORDER BY id
        """)
        
        participantes_nuevos = [(row[0], row[1]) for row in cur.fetchall()]
        
        if len(participantes_nuevos) < 2:
            return jsonify({
                'error': f'Se necesitan al menos 2 participantes nuevos. Actualmente hay {len(participantes_nuevos)}.',
                'participantes_nuevos': len(participantes_nuevos),
                'minimo_requerido': 2,
                'tipo_sorteo': 'nuevos'
            }), 400
        
        # Algoritmo de sorteo para participantes nuevos
        participantes_ids = [p[0] for p in participantes_nuevos]
        ...
        for i in range(len(participantes_ids)):
            ...
            # Marcar que YA participó en sorteo
            cur.execute("""
            ...
            dador_nombre = next(p[1] for p in participantes_nuevos if p[0] == dador_id)
            receptor_nombre = next(p[1] for p in participantes_nuevos if p[0] == receptor_id)
        ...
        return jsonify({
            'success': True,
            'mensaje': f'✅ Sorteo realizado para {len(participantes_nuevos)} participantes NUEVOS',
            'total_participantes': len(participantes_nuevos),
            'tipo_sorteo': 'nuevos',
...
def obtener_estadisticas():
    """Obtiene estadísticas del sistema - VERSIÓN CORREGIDA"""
    conn = psycopg2.connect(**DB_CONFIG)
    cur = conn.cursor()
    ...
    try:
        ...
        # Participantes que YA han participado en sorteo
        cur.execute("SELECT COUNT(*) FROM ia.tb_participantes_ai WHERE participo_sorteo = TRUE")
        participantes_con_sorteo = cur.fetchone()[0]
        
        # Participantes NUEVOS (sin sorteo)
        participantes_nuevos = total_participantes - participantes_con_sorteo
        
        return jsonify({
            'total_participantes': total_participantes,
            'total_asignaciones': total_asignaciones,
            'participantes_con_sorteo': participantes_con_sorteo,
